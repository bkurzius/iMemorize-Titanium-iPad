...
//Rare exceptions to indicate a bug in the _imemorizeipadjune2011 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_imemorizeipadjune2011 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_imemorizeipadjune2011]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.imemorizeipadjune2011.module"

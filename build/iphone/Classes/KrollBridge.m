...
NSString * iMemorizeiPadJune2011$ModuleRequireFormat = @"(function(exports){"
...
@implementation iMemorizeiPadJune2011Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_imemorizeipadjune2011);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create iMemorizeiPadJune2011 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "iMemorizeiPadJune2011" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_imemorizeipadjune2011 = [[iMemorizeiPadJune2011Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_imemorizeipadjune2011];
	
	NSString *_imemorizeipadjune2011NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _imemorizeipadjune2011NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_imemorizeipadjune2011 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:iMemorizeiPadJune2011$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.imemorizeipadjune2011.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.imemorizeipadjune2011.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}

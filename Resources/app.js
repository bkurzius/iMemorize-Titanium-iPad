...
var App = {
	...
	init: function(){
		...
		if(TiUtils.isAndroid()){
			...
			App.data.height = Ti.Platform.displayCaps.platformHeight - App.data.iPhoneStatusBarHeight;
		}
...
App.draw = function(){
	...
	if (!TiUtils.isAndroid()) {
		...
		App.ui.buttonbar = Ti.UI.createView({
			backgroundColor: '#000000',
			height: 40
		});
		App.ui.prevQuoteBtn = Titanium.UI.createButton({
			title: "<",
			height: 40,
			width: 40,
			left: 20,
		...
		App.ui.hideWordsBtn = Titanium.UI.createButton({
			title: "Hide Words",
			height: 40,
			width: 100,
			left: 60,
		...
		App.ui.showAllWordsBtn = Titanium.UI.createButton({
			title: "Show All",
			height: 40,
			width: 100,
			left: 160,
		...
		App.ui.nextQuoteBtn = Titanium.UI.createButton({
			title: ">",
			height: 40,
			width: 40,
			left: 260,
		...
		App.ui.buttonbar.add(App.ui.prevQuoteBtn);
		App.ui.buttonbar.add(App.ui.hideWordsBtn);
		App.ui.buttonbar.add(App.ui.showAllWordsBtn);
		App.ui.buttonbar.add(App.ui.nextQuoteBtn);
		App.ui.prevQuoteBtn.addEventListener("click", function(){
			Ti.App.fireEvent('loadPrevQuote', {});
		});
		App.ui.hideWordsBtn.addEventListener("click", function(){
			Ti.App.fireEvent('hideWords', {});
		});
		App.ui.showAllWordsBtn.addEventListener("click", function(){
			Ti.App.fireEvent('showAllWords', {});
		});
		App.ui.nextQuoteBtn.addEventListener("click", function(){
...
App.openUiWindow = function(_url,_closeBtn,_html){
	...
	App.ui.uiWin = Ti.UI.createView({backgroundColor:'#000000', width:tempWidth,height:tempHeight});
	...
	if(_closeBtn){
		...
		var closeBtn = Ti.UI.createButton({
			...
			borderRadius:10,
			color:'#545454',
			font:{fontSize:16,fontFamily:App.styles.fontFamily,fontWeight:'bold'}
		});
...
App.buildSavedQuotes = function(){
	...
	Ti.App.fireEvent(App.data.action,{data:contents.text});
};
...
App.handleButtonbarClick = function(e){
	if(e.index===0){Ti.App.fireEvent('loadPrevQuote',{});}
	if(e.index===1){Ti.App.fireEvent('hideWords',{});}
	if(e.index===2){Ti.App.fireEvent('showAllWords',{});}
	if(e.index===3){Ti.App.fireEvent('loadNextQuote',{});}
};
...
App.handleTitaniumEnterQuote = function(){
	Ti.API.info(">>>>>>>>>>App.handleTitaniumEnterQuote");	
	Ti.App.Properties.setString("quoteEditAction_preference","add");
	//openEditMenu();
	App.ui.webview.visible = true;
	App.showQuoteEditor();
...
App.handleTitaniumAppLoaded = function(e){
	...
	Ti.App.fireEvent('networkOnline', {online:tmpOnline});
};
...
App.sendMemorizedArray = function(){
	...
	if (!f.exists()) {
		...
		Ti.App.fireEvent("MemorizedQuotesArraySent",{data:contents.text});
	}
};
...
App.handleTitaniumOrientationChange = function(e){
	...
	if(TiUtils.isAndroid()){
		...
		App.data.height = TiUtils.getAppHeight() - App.data.iPhoneStatusBarHeight;
	}	
	App.data.width = TiUtils.getAppWidth(); //- App.data.width;
	...
	App.updateLayout();
	Ti.App.fireEvent('orientationChangeEvent', {orientation:App.data.orientation});
};
...
App.sendQuoteToServer = function(_text,_author,_source,_language) {
	...
	var data = {'text':_text,'author':_author,'source':_source,'language':_language};
...
App.showEmailQuoteDialog = function(_quoteId){
	...
	if(App.data.currentQuote.introText!==undefined){myQuote = App.data.currentQuote.introText + "\n";}	
...
App.showGameButtons = function(){
	...
	Ti.API.info('this is android: ' + TiUtils.isAndroid());
	//if (!TiUtils.isAndroid()) {
